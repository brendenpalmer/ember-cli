...
const { _getEmptyTree } = require('../../broccoli/merge-trees');
const { TARGET_INSTANCE } = require('./target-instance');

const NO_OP_FN = () => undefined;

function customTreeFor(instance, treeType) {
  if (treeType === 'app' || treeType === 'public') {
    return instance.treeFor(treeType);
  }

  return _getEmptyTree();
}
        ...
        if (property !== 'constructor' && typeof targetCacheEntry[TARGET_INSTANCE][property] === 'function') {
          ...
          return function _originalAddonPropMethodWrapper() {
            if (property === 'treeFor') {
              ...
              return customTreeFor(targetCacheEntry[TARGET_INSTANCE], treeType);
            }
